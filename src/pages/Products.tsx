          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-20">
            {filtered.map((p, i) => (
              <FadeIn key={p.slug} delay={i * 60} className="h-full">
                <div className="group bg-card rounded-2xl border border-border/50 overflow-visible hover:border-primary/30 transition-all hover:shadow-elevated relative h-full flex flex-col">
                  {/* Floating product image */}
                  <div className="flex items-end justify-center -mt-12 mb-2 px-4 flex-1">
                    <img
                      src={p.image}
                      alt={p.name}
